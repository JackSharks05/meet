...
// Google Cloud Tasks powered the email-reminder system, which is disabled in
// this fork. Without a service-account key (SERVICE_ACCOUNT_KEY_PATH) we skip
// client creation entirely so the server starts with no GCP dependency.
// Callers guard against a nil TasksClient.
func InitTasks() func() {
	ctx := context.Background()

	credsFile := os.Getenv("SERVICE_ACCOUNT_KEY_PATH")
	if credsFile == "" {
		logger.StdErr.Println("SERVICE_ACCOUNT_KEY_PATH not set; Cloud Tasks (email reminders) disabled")
		return func() {}
	}
...
func CreateEmailTask(email string, ownerName string, eventName string, eventId string) []string {
	// Email reminders disabled in this fork when Cloud Tasks isn't configured.
	if TasksClient == nil {
		return []string{}
	}
