...
// Sign-in (session creation) is registered only on the admin listener; the
// public (respondent) listener exposes nothing that can create a session.
func InitAuth(router *gin.RouterGroup, admin bool) {
	authRouter := router.Group("/auth")

	// Read-only status is available everywhere (returns 401 for guests).
...
// isOperatorAllowed reports whether the given email may sign in (and therefore
// create/manage events). If ADMIN_EMAILS is empty there is no restriction;
// otherwise the email must appear in the comma-separated allowlist.
func isOperatorAllowed(email string) bool {
...
func signInHelper(c *gin.Context, token auth.TokenResponse, tokenOrigin models.TokenOriginType, calendarType models.CalendarType, timezoneOffset int) models.User {
	...
	// Modified by Jack de Haan, 2026 (meet fork of Timeful). See NOTICE.
	// Restrict who may sign in (and thus create/manage events) to the operator
	// allowlist. Callers must check c.IsAborted() before writing a response.
	if !isOperatorAllowed(email) {
		logger.StdErr.Printf("Rejected sign-in for non-allowlisted email: %s\n", email)
