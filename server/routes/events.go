...
// Respondent-facing routes are registered on both listeners. Event management
// (create/edit/delete/duplicate/archive/decline + calendar overlay) is
// registered ONLY on the admin listener, and create/edit now require auth, so
// the public listener cannot create or modify events at all.
func InitEvents(router *gin.RouterGroup, admin bool) {
	...
	// Event-management routes (admin listener only — bound to the tailnet).
	// Modified by Jack de Haan, 2026 (meet fork of Timeful). See NOTICE.
	// createEvent no longer requires a meet sign-in: the operator creates events
	// from Mensa over the tailnet, so the protection is network isolation (this
